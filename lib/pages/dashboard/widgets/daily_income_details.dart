...
class _DailyIncomeDetailsState extends State<DailyIncomeDetails> {
  ...
  Future<void> fetchDailyIncomeData() async {
    ...
    try {
      ...
      final uri = Uri.parse(
        'http://127.0.0.1:8000/api/revenuedetails/filterbydays/',
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map<String, dynamic> && data[widget.location] != null) {
          setState(() {
            dailyIncomeData = List.from(data[widget.location]);
          });
        } else {
    ...
    return Row(
      children: [
        _buildFilterButton(
          icon: Icons.event,
          label:
              DateFormat('MMMM', 'id_ID').format(DateTime(2022, selectedMonth)),
        ...
        return AlertDialog(
          ...
          content: SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                return ListTile(
                  title: Text(DateFormat('MMMM', 'id_ID')
